<template>
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">Workflow</h1>
    ...
    <div v-if="loading" class="text-center text-gray-400 py-12">กำลังโหลด...</div>
    <div v-else-if="workflows.length === 0" class="text-center text-gray-400 py-12">ไม่มีข้อมูล Workflow</div>
    <div v-else class="space-y-4">
      <div v-for="wf in workflows" :key="wf.id" class="bg-white rounded-xl border border-gray-200">
        <div class="flex items-center justify-between p-5">
          <div class="flex items-center gap-3">
            <span :class="wf.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'"
              class="px-2 py-0.5 rounded-full text-xs font-medium">
              {{ wf.isActive ? 'Active' : 'Inactive' }}
            </span>
            <div>
              <p class="font-semibold text-gray-900">{{ wf.name }}</p>
          ...
          <button @click="openEdit(wf)"
            class="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-900 border border-gray-200 hover:border-gray-300 rounded-lg transition-colors">
            แก้ไข
          </button>
        </div>
        <div v-if="wf.steps.length" class="border-t border-gray-100 px-5 pb-4 pt-3">
          <div class="space-y-2">
            <div v-for="step in wf.steps" :key="step.id"
              ...
              <span class="text-gray-500">{{ APPROVER_TYPE_LABEL[step.approverType] ?? step.approverType }}</span>
        ...
        <form @submit.prevent="save" class="p-6 space-y-5">
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-sm font-medium text-gray-700">ขั้นตอนการอนุมัติ</label>
            ...
            <div class="space-y-3">
              <div v-for="(step, i) in form.steps" :key="i"
                ...
                <div class="flex items-center justify-between">
                  <span class="text-xs font-semibold text-gray-500">ขั้นตอนที่ {{ i + 1 }}</span>
                ...
                <input v-model="step.stepName" type="text" required placeholder="ชื่อขั้นตอน"
                  class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" />
          ...
          <div class="flex gap-3 pt-1">
            <button type="button" @click="closeModal"
              class="flex-1 px-4 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50 transition-colors">
              ยกเลิก
            </button>
