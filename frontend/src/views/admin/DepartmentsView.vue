<template>
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">จัดการแผนก</h1>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">ชื่อแผนก</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">ผู้จัดการแผนก</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">จำนวนผู้ใช้</th>
            <th class="px-4 py-3"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-if="departments.length === 0">
            <td colspan="4" class="text-center text-gray-400 py-10">ยังไม่มีแผนก</td>
          </tr>
          <tr v-for="d in departments" :key="d.id" class="hover:bg-gray-50">
            <td class="px-4 py-3 font-medium text-gray-900">{{ d.name }}</td>
            <td class="px-4 py-3 text-gray-600">{{ d.managerName || '-' }}</td>
            <td class="px-4 py-3 text-gray-600">{{ d.userCount }} คน</td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div class="px-6 py-4 border-b border-gray-100">
          <h2 class="font-semibold text-gray-900">{{ editingDept ? 'แก้ไขแผนก' : 'เพิ่มแผนกใหม่' }}</h2>
        </div>
        <div class="px-6 py-4 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">ผู้จัดการแผนก</label>
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          <button @click="closeModal" class="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
            ยกเลิก
          </button>
