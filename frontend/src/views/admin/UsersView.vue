      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th class="text-left px-4 py-3 font-semibold text-gray-600">อีเมล</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">แผนก</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">บทบาท</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600">สถานะ</th>
            <th class="px-4 py-3"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-if="users.length === 0">
            <td colspan="6" class="text-center text-gray-400 py-10">ยังไม่มีผู้ใช้งาน</td>
          </tr>
          <tr v-for="u in users" :key="u.id" class="hover:bg-gray-50">
            <td class="px-4 py-3 font-medium text-gray-900">{{ u.username }}</td>
            <td class="px-4 py-3 text-gray-600">{{ u.email }}</td>
            <td class="px-4 py-3 text-gray-600">{{ u.departmentName }}</td>
            <td class="px-4 py-3">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div class="px-6 py-4 border-b border-gray-100">
          <h2 class="font-semibold text-gray-900">{{ editingUser ? 'แก้ไขผู้ใช้งาน' : 'เพิ่มผู้ใช้งานใหม่' }}</h2>
        </div>
        <div class="px-6 py-4 space-y-4">
          ...
          <div v-if="editingUser" class="flex items-center gap-3">
            ...
            <label for="isActive" class="text-sm font-medium text-gray-700">เปิดใช้งาน</label>
          </div>
